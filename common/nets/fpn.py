...
class FPN(nn.Module):
    ...
    def forward(self, x):
        ...
        feat = self.toplayer(c5)
        out['p5'] = self.smooth5(feat)
        feat = self._upsample_add(feat, self.latlayer4(c4))
        out['p4'] = self.smooth4(feat)
        feat = self._upsample_add(feat, self.latlayer3(c3))
        out['p3'] = self.smooth3(feat)
        feat = self._upsample_add(feat, self.latlayer2(c2))
        out['p2'] = self.smooth2(feat)
